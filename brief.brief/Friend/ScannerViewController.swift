...
import FirebaseAuth
import FirebaseAnalytics  // Make sure to import FirebaseAnalytics
    ...
    public class ScannerViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVCaptureMetadataOutputObjectsDelegate, UIAdaptivePresentationControllerDelegate {
        ...
        public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
            ...
            guard let qrcodeImg = info[.originalImage] as? UIImage else {
                didFail(reason: .galleryError)  // Call the error function with the new enum case for gallery error
        ...
        private lazy var qrCodeLabel: UILabel = {
            let label = UILabel()
            label.text = NSLocalizedString("Mon code QR", comment: "QR Code Label")
        ...
        private lazy var openGalleryButton: UIButton = {
            let button = UIButton(type: .system)
            button.setTitle(NSLocalizedString("Télécharger un code QR", comment: "Open Gallery Button"), for: .normal)
        ...
        private func setupConstraints() {
                    NSLayoutConstraint.activate([
                        qrCodeLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor),
                        qrCodeLabel.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -20),
                        openGalleryButton.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -20),
                        openGalleryButton.topAnchor.constraint(equalTo: view.topAnchor, constant: 20)
                    ])
        }
        ...
        private func setupSession() {
            ...
            view.bringSubviewToFront(openGalleryButton)
        ...
        func found(_ result: ScanResult) {
            ...
            // Call your add friend method here
            addFriendImmediately(scannedUserID: result.string)
            
            parentView.completion(.success(result))
        }

        
        func addFriendImmediately(scannedUserID: String) {
            guard let currentUserID = Auth.auth().currentUser?.uid else {
                showAlert(title: "Error", message: "Could not get current user.")
                return
            }
            
            if scannedUserID.isEmpty || scannedUserID == currentUserID {
                showAlert(title: "Invalid QR Code", message: "Le code QR n'est pas valide.")
                return
            }
            
            let currentUserFriendsRef = db.collection("Friends").document(currentUserID)
            let scannedUserFriendsRef = db.collection("Friends").document(scannedUserID)
            
            currentUserFriendsRef.getDocument { (currentDocument, error) in
                if let error = error {
                    print("Error getting document: \(error)")
                    self.showAlert(title: "Error", message: "Could not complete the operation.")
                    return
                }
                
                var currentFriends = currentDocument?.data()?["friendsList"] as? [String] ?? []
                
                scannedUserFriendsRef.getDocument { (scannedDocument, error) in
                    if let error = error {
                        print("Error getting document: \(error)")
                        self.showAlert(title: "Error", message: "Could not complete the operation.")
                        return
                    ...
                    var scannedFriends = scannedDocument?.data()?["friendsList"] as? [String] ?? []
                    
                    if currentFriends.contains(scannedUserID) {
                        self.showAlert(title: "Déjà ami", message: "Vous êtes déjà ami avec l'utilisateur.")
                    } else if scannedFriends.contains(currentUserID) {
                        self.showAlert(title: "Déjà ami", message: "L'utilisateur est déjà votre ami.")
                    } else {
                        currentFriends.append(scannedUserID)
                        scannedFriends.append(currentUserID)
                        
                        currentUserFriendsRef.setData(["friendsList": currentFriends], merge: true) { (error) in
                            if let error = error {
                                print("Error setting data: \(error)")
                                self.showAlert(title: "Error", message: "Could not add friend.")
                                return
                            }
                        }
                        
                        scannedUserFriendsRef.setData(["friendsList": scannedFriends], merge: true) { (error) in
                            if let error = error {
                                print("Error setting data: \(error)")
                                self.showAlert(title: "Error", message: "Could not add friend.")
                                return
                        ...
                        // Log analytics event
                        Analytics.logEvent("friend_added_immediately", parameters: [
                            "current_user_id": currentUserID,
                            "added_friend_id": scannedUserID
                        ])
                        
                        self.showAlert(title: "Ami ajouté", message: "Vous avez ajouté un nouvel ami.")
                    }
                }
            }
        }
        ...
        func didFail(reason: ScanError) {
            ...
            case .galleryError:
                    message = "Failed to read QR code from gallery."  // New error message
                }
